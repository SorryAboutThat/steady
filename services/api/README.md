...
![Run Tests](https://github.com/SorryAboutThat/steady/workflows/Tests/badge.svg)
...
- `MONGO_URI` - MongoDB URI to connect to, defaults to `mongodb://localhost/steady_dev`
...
- `ADMIN_EMAIL` - Default dashboard admin user `info@paylo.dev`
- `ADMIN_PASSWORD` - Default dashboard admin password `[change me]`
- `APP_NAME` - Default product name to be used in emails `steady`
- `APP_URL` - URL for app defaults to `http://localhost:2200`
- `POSTMARK_FROM` - Reply email address `no-reply@paylo.dev`
...
```
docker build -t steady-api-jobs -f Dockerfile.jobs .
```
