...
- [steady Deployment](#bedrock-deployment)
  - [Setup](#setup)
...
- Use `gcloud config configurations create steady` and authorize Google Cloud `gcloud auth login`
...
```bash
gcloud projects create steady-staging --name="steady Staging"
...
```bash
./deployment/scripts/provision_gcloud staging steady-staging
```
